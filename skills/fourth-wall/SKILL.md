---
name: fourth-wall
description: Coordinate substantial work through a bounded native Codex MultiAgentV2 team. Read before the first subagent spawn or orchestration action. Every source-changing Fourth Wall campaign requires its ordinary root to dispatch and obtain an independent Review of a frozen coherent candidate before accepted, integration-ready, or complete status, regardless of apparent triviality; the root never self-reviews. This gate does not govern routine pair-programming, read-only, evidence-only, monitoring, or no-code work. Use for evidence-first triage, parallel implementation slices, persistent specialist reuse, adversarial review, runtime QA, deployment, task messaging, long event-driven waits, goals, synchronization, and recovery. Do not use for routine single-agent work or unrelated top-level Desktop tasks.
---
...
Every Lab, temporary directory, worktree, branch, process, and proof artifact created for a campaign must have a resource-ledger entry with its stable identifier or exact path, current owner task, and release condition. Record that entry in the dispatch packet or the campaign handoff before creating the resource. The creator retains cleanup custody unless the root explicitly reassigns it; a request for evidence never transfers custody. Reviewers mark evidence consumed and return the resource to its owner rather than deleting or adopting it.

When a slice is integrated, rejected, or abandoned, the current owner releases each resource deliberately: destroy owned Container Labs, remove only exact released temporary paths and verify they disappeared, and stop owned processes. Prune a merged or abandoned worktree or branch only after confirming no active task still owns it and no useful unmerged work remains. The root verifies the ledger and records any retained resources before terminal campaign close. Never create an extra branch, worktree, clone, Lab, or immutable proof copy merely to make Review possible; the default review topology remains one shared worktree and an immutable candidate commit.
...
Every source-changing Fourth Wall campaign requires one mandatory terminal aggregate Review against a frozen coherent candidate before the root may claim `accepted`, `integration-ready`, or `complete`, regardless of diff size or apparent triviality. The ordinary campaign root owns dispatching the Reviewer and obtaining the independent verdict; it never self-reviews. This is one campaign-level review, not one review per Worker.
...
The default review topology is one shared worktree. Do not create extra branches, temporary worktrees, clones, or proof copies, or require per-Worker commits, merely to make review possible. Review is a checkpoint against an immutable candidate, not an acceptance verdict on a moving worktree.
...
3. **Commit and freeze.** The root creates one coherent stable-candidate commit. This commit is the Review/QA target. Git integration stays with the root by default, but an exact current-task user authorization may delegate one scoped operation to a named serialized owner under [references/coordination-loop.md](references/coordination-loop.md); acceptance always remains with the root. No edits to reviewed surfaces land through Review/QA.
...
6. After parallel edits stabilize, the root quiesces parallel writers. If integration repair or a serial build/test/fix lane is needed, activate exactly one integration Worker while all other writers remain quiescent; after it finishes, re-quiesce reviewed surfaces, integrate the final aggregate state, and run focused aggregate validation. Git mutations stay with the root by default; exact current-task user authority may delegate one scoped operation under the coordination contract, while acceptance remains with the root.
7. Commit one coherent stable candidate after that validation, excluding unrelated shared-worktree changes; do not create per-Worker commits or extra branches/worktrees for review.
   When task-launch ambiguity or later evidence reveals duplicate roots pursuing the same outcome and base, pause further external publication until the coordinating root names one canonical publisher and compares the exact task paths, bases, branches or remote heads, and existing artifacts. Preserve and reuse a useful commit, branch, or PR that a duplicate already published when safe; reconcile disjoint work additively and validate the cumulative result. Never infer duplicate ownership from title similarity alone or automatically stop tasks, force-push, rewrite history, or discard useful work. An exact current-task user authorization may permit one named serialized owner to perform the scoped Git operation under the execution-safety procedure. This is a recovery gate for known or suspected duplication, not a routine semantic-deduplication scan.
8. For every source-changing Fourth Wall campaign, the root dispatches and obtains the mandatory terminal aggregate Review against the exact checkpoint packet; dispatch an additional Review before downstream consumption only for a deliberately declared source-level shared or public contract checkpoint, and dispatch QA as needed for runtime proof. Return findings to the persistent specialist, then use an additive repair commit, same-Reviewer source re-review, and same-QA-owner rerun as described above.
9. Finish aggregate validation, verify the resource ledger and release every resource that is no longer needed, then make the explicit decision when possible; record the campaign terminal disposition as `accepted`, `rejected`, `blocked`, or `abandoned`.
...
- The root owns Git integration by default and always owns task-graph shape, cross-slice decisions, evidence acceptance, and final completion. Exact current-task user authority may delegate one scoped Git operation to a named serialized owner under the coordination contract.
