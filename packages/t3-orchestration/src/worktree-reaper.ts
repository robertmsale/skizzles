...
// Child binds cwd to the already-opened artifact (fd 3) before exec. Cargo and
// rm receive that inode as "." / its children. Flutter clean has no --build-dir
// and would otherwise re-lookup spec.artifactName from the crate, so the child
// empties fd 3 first, then execs `flutter clean .` on that same inode.
