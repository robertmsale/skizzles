...
describe("selection safety", () => {
  ...
  test("preserves the exact provider option values", () => {
    ...
    expect(requireSelection({ instanceId: "cursor", model: "grok-4.6", options: [{ id: "reasoning", value: "high" }] })).toEqual({
      instanceId: "cursor", model: "grok-4.6", options: [{ id: "reasoning", value: "high" }],
    });
  });
});
