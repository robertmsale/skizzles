...
A genuine "HTTP failed in the app you are debugging" still goes through. Turns that already emitted a tool call are not swallowed.

On a match the shim drops that assistant text and the matching `session/prompt` result, then replays the same prompt on the live child. If the child is dead, it respawns, re-runs `initialize` / `authenticate` / `session/load` only as far as needed, and does not invent a Cursor-internal session. After the retry budget it returns a JSON-RPC error (`-32000`) so T3 never sees the flake as last words.
