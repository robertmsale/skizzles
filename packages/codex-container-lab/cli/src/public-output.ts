...
export type RedactedPublicText = { text: string; contentRedacted: boolean };

/**
 * Redact one already-framed public record and report whether any content was
 * intentionally removed or replaced.  Callers that capture multiple records
 * must establish their record boundary before invoking this function; the
 * conservative unquoted-path rule intentionally consumes the remainder of a
 * single record so an ambiguous path suffix cannot escape.
 */
export function redactPublicTextWithMetadata(
  value: string,
  maxBytes = 2_000,
  maxLines = 8,
  ...
  // characters, so replace the entire remaining record from the first
  // absolute-path signature. Over-redaction is safer than leaking a suffix.
  const quotedAndTagsRedacted = controlsRedacted
