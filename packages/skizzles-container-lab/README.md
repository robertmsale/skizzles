...
1. First follow any committed consuming-repository guidance for a project-owned Container Lab wrapper, especially when it supplies manifest `secret_environment` values. Otherwise, from the Skizzles root run `just lab-help` to use the generic bundled launcher without PATH wiring. Read [docs/installation.md](docs/installation.md) before optional host wiring.
