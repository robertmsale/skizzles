...
- Keep active-owner labs with missing, malformed, or future activity leases intact. For a consistent active owner, a valid lease older than seven days is a temporary-storage retention signal only; it is not ownership or thread-liveness proof. Never infer descendant state from a parent row.
