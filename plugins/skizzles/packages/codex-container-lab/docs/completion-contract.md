...
- When Compose provisioning fails, capture a bounded, redacted per-lab terminal summary before exact-label Docker cleanup. Preserve it as optional version-1 metadata and an owner-scoped opaque diagnostic descriptor; retrieval may expose only bounded transcript text and structured service state/health/exit fields, never a filesystem path. Capture or transcript writes are best effort and must not mask the original failure or prevent cleanup. Explicit destroy and archive reaping remove the artifact.
