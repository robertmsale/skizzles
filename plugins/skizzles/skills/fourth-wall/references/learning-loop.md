...
| `review_required_checkpoints` | the terminal aggregate candidate plus any source-level shared/public contract checkpoint deliberately frozen and declared by root for downstream consumption | checkpoint type, exact target, Review owner, and required scope/oracle; ordinary Worker/dependency handoffs are excluded |
