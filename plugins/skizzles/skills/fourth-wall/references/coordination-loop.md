...
The root owns branch changes, staging, commits, merges, rebases, cherry-picks, stashes, resets, cleans, pushes, and other Git-history mutations unless the user explicitly authorizes an exact current-task action and the root delegates it to one named serialized owner. The delegation must preserve the repository, ownership, worktree, recovery, and target boundaries required by the execution-safety policy. Subagents should use read-only Git inspection by default, but an exact authorized history operation must not be rejected or replaced with additive patch re-expression merely because it is non-additive.
...
By default, the root retains Git mutations; the exact-authority exception above may assign one scoped operation to a named serialized owner. The root always resolves cross-owner decisions, inspects the returned evidence, and accepts or reroutes the result. This is especially important for Flutter, Xcode, Cargo, Gradle, Linux/Xvfb application proof, and package-wide formatters.
