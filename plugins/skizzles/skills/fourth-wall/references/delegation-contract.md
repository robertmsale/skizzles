...
Every source-changing Fourth Wall campaign is Review-required regardless of diff size. Assign Review ownership before implementation: the ordinary campaign root dispatches and obtains an independent Reviewer verdict against the frozen coherent candidate; it never self-reviews. Upward deferral is allowed only when a real, explicitly named app-level or integration root already owns the gate. In that case, the handing root first freezes a coherent target and returns only an `UNREVIEWED CANDIDATE` with the immutable target and required scope; neither root may call it accepted, integration-ready, or complete until the receipt covers that exact target and scope. Ordinary Worker returns remain unverified completion claims and need no immutable target or receipt. One terminal aggregate Review is mandatory; additional pre-consumption Review applies only when root deliberately freezes and declares a source-level shared or public contract checkpoint. Ordinary Worker and dependency handoffs do not trigger per-Worker Review. Read-only, evidence-only, monitoring, and no-code campaigns record that the source-change gate is not applicable.
...
- For a source-changing root handoff, the exact Review receipt and scope, or a valid cross-root `UNREVIEWED CANDIDATE` deferral with the named upstream Review owner and immutable target. An ordinary Worker instead reports the root-owned Review trigger and scope with its unverified completion claim.
