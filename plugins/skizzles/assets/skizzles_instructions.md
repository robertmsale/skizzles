...
- Choose the safest efficient editing method: scoped patches, codemods, or generators are all acceptable when constrained to the owned files. Inspect the resulting diff and workspace status. Never overwrite, reset, clean, discard, rewrite history, publish releases, or alter production without exact authority. Additive commits and non-force pushes from a verified dedicated non-default task branch governed by the closeout contract below carry standing authority and do not require a separate permission question. When the user explicitly authorizes another exact scoped Git operation, follow that authority after verifying its repository, ownership, worktree, recovery, and target boundaries; do not substitute additive re-expression merely because the authorized operation changes history.
...
- Before source-changing work, verify that the checkout is an exclusively owned task worktree, inspect Git status, and identify the task-owned baseline. Work only on a dedicated non-default task branch. If detached or on a default/protected branch, create or switch to a task branch only after exclusive worktree ownership is verified; otherwise create a separate task worktree or stop and report the collision. Preserve and do not stage pre-existing or concurrent foreign changes. If worktree, branch, or change ownership cannot be established safely, stop before editing.
- Treat task-owned clean Git state as a mandatory end-of-turn invariant for source-changing work. Before any final handoff, commit every intentional task-owned change at coherent forward-progress boundaries and verify no task-owned modification or untracked artifact remains; do not ask permission to commit. If foreign changes keep aggregate `git status` dirty, leave them untouched and report them precisely rather than committing or deleting them.
- When a verified configured remote exists, non-force push the dedicated non-default task branch and create or update its draft pull request so completed progress is durable and visible; do not ask permission for these ordinary additive task-branch operations. Never push a default/protected branch under this standing authority. Never mix unrelated work, secrets, or generated artifacts whose canonical source was not updated into a commit.
- Never bypass, disable, or skip a commit hook without explicit user permission. If a hook reports a validation defect, fix the defect and retry the commit. If the hook cannot complete because of broken tooling or another condition outside the task's control, exhaust reasonable repairs, then stop and report the exact blocker and remaining dirty state; a verified externally blocked hook is the only acceptable reason for task-owned changes to remain uncommitted at handoff.
