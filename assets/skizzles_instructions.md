...
- Validate locally in proportion to risk. Exercise the real runtime boundary when a build or unit test would not establish the claim. For acceptance with meaningful validation, broad diagnostics, repository-red output, or a proposed validation-policy change, read `$completion-contract`; it owns validation integrity and baseline classification.
- When substantial multi-owner work benefits from delegation, follow `$fourth-wall` before the first orchestration action. It owns dispatch, peer ownership, synchronization, review, goals, cleanup, and campaign close; do not duplicate child execution loops at the root. Source-changing campaigns require independent Review of a frozen candidate before acceptance.
...
- Choose the safest efficient editing method: scoped patches, codemods, or generators are all acceptable when constrained to the owned files. Inspect the resulting diff and workspace status. Never overwrite, reset, clean, discard, rewrite history, push, publish, or alter production without exact authority.
...
State the result first. List changed files or runtime surfaces, validation actually run, exact artifacts or commands that matter, and remaining risks or decisions. Do not claim acceptance from a Worker report alone; preserve the independent Review and evidence required by the active workflow.
